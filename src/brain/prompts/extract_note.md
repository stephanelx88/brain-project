...
### Triple rules (same as session)

- **Valid predicates**: `worksAt`, `workedAt`, `knows`, `manages`, `reportsTo`, `partOf`, `locatedIn`, `builds`, `uses`, `involves`, `relatedTo`, `about`, `decidedOn`, `learnedFrom`, `contradicts`
- **Only explicit** — never infer. Emit `"triples": []` if no clear typed relationship in the note.

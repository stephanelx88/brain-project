...
def isolated_brain(tmp_path, monkeypatch):
    """Provide an isolated tmp vault path + a tmp HOME so writes to the
    user's shell rc are sandboxed too. Without this a real `brain init
    --yes` would clobber the developer's own vault and ~/.zshrc on
    `pytest` invocations."""
    brain = tmp_path / ".brain"
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    # Pre-create rc so _persist_brain_dir_to_shell_rc has something to append to.
    (fake_home / ".zshrc").write_text("# fake rc for tests\n")
    monkeypatch.setenv("HOME", str(fake_home))
    monkeypatch.setenv("SHELL", "/bin/zsh")
    # Wipe any inherited BRAIN_DIR so _detect_existing_vault returns None.
    monkeypatch.delenv("BRAIN_DIR", raising=False)
    return brain


def _config_path(vault: Path) -> Path:
    return vault / "brain-config.yaml"
...
class TestMergeConfig:
    ...
    def test_merge_preserves_unrelated_existing_keys(self, isolated_brain):
        ...
        _config_path(isolated_brain).write_text(yaml.safe_dump({
            "version": "9.9.9",
            "auto_commit": False,
        ...
        cfg = init_mod._merge_config(
            isolated_brain, preset, preset["entity_types"],
    ...
    def test_merge_skip_provider_does_not_overwrite(self, isolated_brain):
        isolated_brain.mkdir()
        _config_path(isolated_brain).write_text(yaml.safe_dump({"llm_provider": "ollama"}))
        preset = load_preset("developer")
        cfg = init_mod._merge_config(
            isolated_brain, preset, preset["entity_types"],
            {"name": "X", "role": "Y", "field": "Z"},
    ...
    def test_merge_handles_unparseable_existing_config(self, isolated_brain):
        isolated_brain.mkdir()
        _config_path(isolated_brain).write_text("::: not valid yaml :::\n  - [")
        preset = load_preset("developer")
        cfg = init_mod._merge_config(
            isolated_brain, preset, preset["entity_types"],
            {"name": "X", "role": "Y", "field": "Z"},
        ...
        assert _config_path(isolated_brain).with_suffix(".yaml.bak").exists()
    ...
    def test_merge_handles_yaml_that_parses_to_non_dict(self, isolated_brain):
        ...
        isolated_brain.mkdir()
        _config_path(isolated_brain).write_text(":::not yaml{[")
        preset = load_preset("developer")
        cfg = init_mod._merge_config(
            isolated_brain, preset, preset["entity_types"],
            {"name": "X", "role": "Y", "field": "Z"},
    ...
    def test_merge_handles_yaml_list_at_root(self, isolated_brain):
        ...
        isolated_brain.mkdir()
        _config_path(isolated_brain).write_text("- one\n- two\n")
        preset = load_preset("developer")
        cfg = init_mod._merge_config(
            isolated_brain, preset, preset["entity_types"],
            {"name": "X", "role": "Y", "field": "Z"},
...
class TestRenderWhoIAm:
    def test_renders_full_identity_block(self, isolated_brain):
        ...
        init_mod._render_who_i_am(isolated_brain, preset, identity, force=False)
    ...
    def test_skips_existing_unless_forced(self, isolated_brain):
        ...
        preset = load_preset("doctor")
        init_mod._render_who_i_am(isolated_brain, preset, {"name": "x", "role": "y", "field": "z"}, force=False)
    ...
    def test_force_overwrites_existing(self, isolated_brain):
        ...
        existing.write_text("MY OWN NOTES")
        preset = load_preset("developer")
        init_mod._render_who_i_am(isolated_brain, preset, {"name": "Son", "role": "Dev", "field": "SE"}, force=True)
        assert existing.read_text() != "MY OWN NOTES"
...
class TestEntityDirs:
    def test_creates_each_preset_folder(self, isolated_brain):
        ...
        init_mod._create_entity_dirs(isolated_brain, preset["entity_types"])
...
class TestPersistBrainDir:
    def test_appends_export_to_shell_rc(self, isolated_brain):
        ...
        body = rc.read_text()
        assert "# >>> brain vault >>>" in body
        assert f'export BRAIN_DIR="{isolated_brain}"' in body
        assert "# <<< brain vault <<<" in body

    def test_replaces_existing_block_on_rerun(self, isolated_brain, tmp_path):
        init_mod._persist_brain_dir_to_shell_rc(isolated_brain)
        new_vault = tmp_path / "new-vault"
        rc = init_mod._persist_brain_dir_to_shell_rc(new_vault)
        body = rc.read_text()
        assert body.count("# >>> brain vault >>>") == 1, "must not duplicate the block"
        assert f'export BRAIN_DIR="{new_vault}"' in body
        assert str(isolated_brain) not in body


# ─────────────────────────────────────────────────────────────────────────
# Non-interactive end-to-end (--yes, --no-install, --vault)
# ─────────────────────────────────────────────────────────────────────────
class TestMainEntryPoint:
    def test_yes_with_no_install_runs_clean(self, isolated_brain):
        rc = init_mod.main(["--yes", "--no-install", "--preset", "doctor",
                            "--vault", str(isolated_brain)])
        assert rc == 0
        cfg = yaml.safe_load(_config_path(isolated_brain).read_text())
        assert cfg["preset"] == "doctor"
    ...
    def test_yes_without_vault_fails(self, isolated_brain):
        # No --vault and no $BRAIN_DIR — should refuse.
        rc = init_mod.main(["--yes", "--no-install"])
        assert rc == 2
    ...
    def test_force_identity_flag_overwrites_on_rerun(self, isolated_brain):
        ...
        init_mod.main(["--yes", "--no-install", "--preset", "doctor",
                       "--vault", str(isolated_brain), "--force-identity"])

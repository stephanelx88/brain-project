...
# Artifacts generated by the brain pipeline — safe to delete when the
# user migrates to a different vault. User-authored content lives in
# `entities/`, `identity/`, `timeline/`, `.git/`, `brain-config.yaml`,
# and any top-level `*.md` — none of which are touched here.
_PURGE_FILES = (
    ".brain.db", ".brain.db-shm", ".brain.db-wal",
    ".brain.db.new", ".brain.db.new-shm", ".brain.db.new-wal",
    ".dedupe.ledger.json",
    ".brain.conf",
    ".entity-names.cache",
    ".claude-settings.brain.json",
    ".cursor-hooks.brain.json",
    "pending_triples.jsonl",
)
_PURGE_DIRS = (".vec", ".brain.rdf", ".extract.lock.d")


def _purge_old_vault_artifacts(old_vault: Path) -> list[str]:
    """Delete brain-generated cache/index files from a retired vault.

    Returns the list of relative paths actually removed. User content
    (entities/, notes, identity/, .git/) is left untouched."""
    import shutil
...
def _maybe_purge_old_vault(new_vault: Path, q=None, assume_yes: bool = False) -> None:
    """If the previous vault differs from `new_vault`, offer to purge
    its brain-generated artifacts so stale indices don't linger.

    Silent no-op when no previous vault is found or when old == new.
    In `--yes` mode the purge runs without a prompt (user opted into
    unattended init). Interactive mode confirms with Y/n (default Y)
    since retained artifacts are what triggered the bug this solves.
    """
    old = _detect_existing_vault()
    if not old or not old.exists():
        return
    try:
        if old.resolve() == new_vault.resolve():
            return
    except OSError:
        return

    # Only purge if at least one artifact exists — avoids an empty
    # prompt when the old dir was already cleaned or never initialised.
    has_artifact = any((old / n).exists() for n in _PURGE_FILES) or any(
        (old / d).exists() for d in _PURGE_DIRS
    )
    if not has_artifact:
        return
    ...
    if assume_yes:
        ...
        do_purge = q.confirm(
            f"Previous vault at {old} has stale brain artifacts "
            f"(.brain.db, .vec/, …). Purge them now? User notes "
            f"and entities/ are left alone.",
            default=True,
        ).unsafe_ask()
    else:
        do_purge = False
    ...
    if not do_purge:
        _info(f"leaving old vault artifacts in place at {old}")
        return

    removed = _purge_old_vault_artifacts(old)
    if removed:
        _ok(f"purged {len(removed)} artifact(s) from {old}: {', '.join(removed)}")
    else:
        _info(f"nothing to purge in {old}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # If the user is moving to a different vault, the previous one's
    # SQLite + vector cache would otherwise linger and confuse future
    # reads (stale facts via mispointed env). Wipe only machine-managed
    # artifacts — user content is left intact.
    _maybe_purge_old_vault(
        vault,
        q=q if not args.yes else None,
        assume_yes=args.yes,
    )
